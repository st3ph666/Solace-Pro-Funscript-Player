...
class SolacePlayerGUI(tk.Tk):
    def __init__(self) -> None:
        ...
        self.theme_name = tk.StringVar(value="Cyber Blue")
        self.time_text = tk.StringVar(value="00:00 / 00:00   •   -00:00")
        self.ble_state = tk.StringVar(value="BLE // IDLE")
        self.playlist_info = tk.StringVar(value="0 video")
        self.section_open = {"media": True, "motion": True, "options": True, "playlist": True}
        ...
        self.playlist_selected_video: Path | None = None
    ...
    TEXTS = {
        "fr": {
            ...
            "playlist_title": "03 // PLAYLIST", "options_title": "04 // OPTIONS",
            "ble_idle": "BLE // INACTIF", "ble_active": "BLE // ACTIF",
            "theme": "THÈME", "playlist_empty": "Aucune vidéo dans la playlist",
        },
        "en": {
            ...
            "playlist_title": "03 // PLAYLIST", "options_title": "04 // OPTIONS",
            "ble_idle": "BLE // IDLE", "ble_active": "BLE // ACTIVE",
            "theme": "THEME", "playlist_empty": "No videos in playlist",
        },
    }
    ...
    def _update_time_text(self) -> None:
        pos = self.graph_position_ms / 1000.0
        dur = self.graph_duration_ms / 1000.0
        remaining = max(0.0, dur - pos)
        self.time_text.set(f"{self._format_clock(pos)} / {self._format_clock(dur)}   •   -{self._format_clock(remaining)}")

    def _toggle_section(self, key: str, body: tk.Widget, button: tk.Button, expand_body: bool = False) -> None:
        opened = not self.section_open.get(key, True)
        self.section_open[key] = opened
        if opened:
            body.pack(fill="both" if expand_body else "x",
                      expand=expand_body, padx=1, pady=(0, 1))
            button.configure(text="▾")
        else:
            body.pack_forget()
            button.configure(text="▸")

    def _make_collapsible(self, parent, key: str, title: str, expand_body: bool = False):
        outer = tk.Frame(parent, bg=COLORS["border"], highlightthickness=0)
        header = tk.Frame(outer, bg=COLORS["panel_alt"], height=36)
        header.pack(fill="x", padx=1, pady=(1, 0))
        header.pack_propagate(False)
        header_label = tk.Label(header, text=title, bg=COLORS["panel_alt"], fg=COLORS["accent"],
                                font=("DejaVu Sans Mono", 10, "bold"), anchor="w", padx=12)
        header_label.pack(side="left", fill="x", expand=True)
        if key == "playlist":
            self.playlist_header_label = header_label
        body = tk.Frame(outer, bg=COLORS["panel"])
        toggle = tk.Button(header, text="▾" if self.section_open.get(key, True) else "▸",
                           bg=COLORS["panel_alt"], fg=COLORS["accent"], relief="flat", bd=0,
                           activebackground=COLORS["panel_alt"], activeforeground=COLORS["accent_hover"],
                           font=("DejaVu Sans Mono", 12, "bold"), cursor="hand2")
        toggle.pack(side="right", padx=8)
        toggle.configure(command=lambda: self._toggle_section(key, body, toggle, expand_body))
        if self.section_open.get(key, True):
            body.pack(fill="both" if expand_body else "x", expand=expand_body, padx=1, pady=(0, 1))
        return outer, body

    def _update_playlist_title(self) -> None:
        if not hasattr(self, "playlist_header_label"):
            return
        folder = self.playlist_folder if self.playlist_folder and self.playlist_folder.is_dir() else None
        if folder:
            self.playlist_header_label.configure(text=f"03 // PLAYLIST // {folder.name}")
        else:
            self.playlist_header_label.configure(text=self.tr("playlist_title"))

    def _playlist_videos(self) -> list[Path]:
        folder = self.playlist_folder if self.playlist_folder and self.playlist_folder.is_dir() else None
        return [v for v in self._folder_videos(folder) if find_script(v)] if folder else []

    def _play_selected_playlist_video(self, _event=None) -> None:
        if not hasattr(self, "playlist_listbox"):
            return

        videos = self._playlist_videos()
        video = self.playlist_selected_video

        if video is None or video not in videos:
            ...
            if not (0 <= idx < len(videos)):
                return
            video = videos[idx]
            self.playlist_selected_video = video
        script = find_script(video)
        if script is None or not script.is_file():
            self.set_status("FUNSCRIPT INTROUVABLE // " + video.name, "warning")
            return

        # Direct selection cancels the current folder queue and immediately plays this item.
        self.playlist_active = False
        self.playlist = []
        self.stop()
        self._set_selected_video(video, announce=False)
        self.set_status(f"LECTURE PLAYLIST // {video.name}", "success")
    ...
    def _refresh_playlist_view(self) -> None:
        ...
        for idx, video in enumerate(videos):
            is_current = bool(self.current_video and video == self.current_video)
            is_selected = bool(self.playlist_selected_video and video == self.playlist_selected_video)
            ...
            self.playlist_listbox.insert("end", marker + video.name)

            # Persistent visual styling, independent of keyboard focus.
            if is_selected:
                self.playlist_listbox.itemconfig(
                    idx,
                    bg=COLORS["accent"],
                    fg="#001018",
                    selectbackground=COLORS["accent_hover"],
        ...
        self.playlist_info.set(f"{len(videos)} video" + ("s" if len(videos) != 1 else ""))

        if not videos:
            self.playlist_listbox.insert("end", self.tr("playlist_empty"))
            if hasattr(self, "playlist_selected_label"):
                self.playlist_selected_label.configure(text="")
            return

        # Keep the chosen row selected and scrolled into view.
        focus_index = selected_index if selected_index is not None else current_index
        if focus_index is not None:
            self.playlist_listbox.selection_clear(0, "end")
            self.playlist_listbox.selection_set(focus_index)
            self.playlist_listbox.activate(focus_index)
            self.playlist_listbox.see(focus_index)

        if hasattr(self, "playlist_selected_label"):
            if selected_index is not None and self.playlist_selected_video:
                self.playlist_selected_label.configure(
                    text="◆ SELECTED // " + self.playlist_selected_video.name,
                    fg=COLORS["accent_hover"],
                )
            elif current_index is not None and self.current_video:
                self.playlist_selected_label.configure(
                    text="▶ PLAYING / READY // " + self.current_video.name,
                    fg=COLORS["accent"],
                )
            else:
                self.playlist_selected_label.configure(text="")

    def _playlist_activate(self, _event=None) -> None:
        if not hasattr(self, "playlist_listbox"):
            return
        sel = self.playlist_listbox.curselection()
        if not sel:
        ...
        # Update source/graph on a single click, but keep the row visibly
        # highlighted instead of rebuilding the list and losing the cue.
        self._set_selected_video(video, announce=False)

        # Restore normal styling to every row, then highlight this one.
        for row in range(self.playlist_listbox.size()):
            self.playlist_listbox.itemconfig(
                row,
                bg="#040A12",
                fg=COLORS["text"],
        ...
        if hasattr(self, "playlist_selected_label"):
            self.playlist_selected_label.configure(
                text="◆ SELECTED // " + video.name,
                fg=COLORS["accent_hover"],
            )

        self.set_status(f"SÉLECTION // {video.name}", "neutral")

    def _set_theme(self, name: str) -> None:
        themes = {
            "Cyber Blue": ("#00CFFF", "#59E6FF"),
            "Matrix Green": ("#35F2A1", "#8CFFD0"),
            "Purple Neon": ("#B56CFF", "#E0B6FF"),
        }
        if name not in themes:
            return
        self.theme_name.set(name)
        COLORS["accent"], COLORS["accent_hover"] = themes[name]
        self.configure_styles()
        self.save_config()
        self.build_ui()
        if self.current_video or self.video_path.get().strip():
    ...
    def _current_funscript_position(self) -> float:
        ...
        if t <= self.graph_actions[0][0]:
            return float(self.graph_actions[0][1])
        if t >= self.graph_actions[-1][0]:
            return float(self.graph_actions[-1][1])
        lo = self.graph_actions[0]
        for hi in self.graph_actions[1:]:
            if t <= hi[0]:
                span = max(1, hi[0] - lo[0])
                ratio = (t - lo[0]) / span
                return lo[1] + (hi[1] - lo[1]) * ratio
            lo = hi
        return float(lo[1])

    def _build_hud_design(self, parent) -> tk.Frame:
        """Live telemetry panel driven by the current Funscript and controls."""
        frame = tk.Frame(
            parent, bg=COLORS["bg"],
            highlightbackground=COLORS["border"], highlightthickness=1
        )
        canvas = tk.Canvas(frame, bg="#020711", highlightthickness=0, bd=0)
        ...
        def redraw(_event=None):
            ...
            pos = max(0.0, min(100.0, self._current_funscript_position()))
            min_pos = max(0.0, min(100.0, self.min_position.get() * 100.0))
            max_pos = max(0.0, min(100.0, self.max_position.get() * 100.0))
            speed = max(0.25, min(2.0, self.speed_multiplier.get()))
            amp = max(0.0, min(100.0, self.amplification.get()))
            playing = self.process is not None and self.process.poll() is None

            # Background technical grid.
            grid = 32
            for x in range(grid, w, grid):
                canvas.create_line(x, 0, x, h, fill="#071426", width=1, tags="hud")
            for y in range(grid, h, grid):
                canvas.create_line(0, y, w, y, fill="#071426", width=1, tags="hud")

            # Top telemetry strip.
            canvas.create_rectangle(14, 14, w-14, 46, outline=border, fill="#04101B", tags="hud")
            state = "ACTIVE" if playing else "STANDBY"
            state_color = success if playing else muted
            canvas.create_text(
                28, 30, anchor="w",
                text=f"CORE // {state}",
                fill=state_color, font=("DejaVu Sans Mono", 9, "bold"), tags="hud"
            )
            canvas.create_text(
                w/2, 30, text=f"FUNSCRIPT POS // {pos:05.1f} %",
                fill=accent, font=("DejaVu Sans Mono", 10, "bold"), tags="hud"
            )
            canvas.create_text(
                w-28, 30, anchor="e",
                text=f"BLE RESPONSE // {speed:0.2f}×",
                fill=accent, font=("DejaVu Sans Mono", 9, "bold"), tags="hud"
            )

            # Left live movement chamber.
            chamber_x1 = 40
            chamber_x2 = max(145, w * 0.38)
            chamber_y1 = 72
            chamber_y2 = h - 54
            canvas.create_rectangle(
                chamber_x1, chamber_y1, chamber_x2, chamber_y2,
                outline=border, width=2, fill="#030B16", tags="hud"
            )
            canvas.create_text(
                chamber_x1 + 12, chamber_y1 + 14, anchor="w",
                text="LIVE MOTION CORE", fill=muted,
                font=("DejaVu Sans Mono", 8, "bold"), tags="hud"
            )

            # Motion rails and permitted range.
            rail_x = (chamber_x1 + chamber_x2) / 2
            rail_top = chamber_y1 + 38
            rail_bottom = chamber_y2 - 24
            canvas.create_line(rail_x, rail_top, rail_x, rail_bottom,
                               fill=border, width=8, tags="hud")
            canvas.create_line(rail_x, rail_top, rail_x, rail_bottom,
                               fill="#0B253A", width=3, tags="hud")

            def py(percent):
                return rail_bottom - (percent / 100.0) * (rail_bottom - rail_top)
            ...
            canvas.create_rectangle(
                rail_x-20, y_max, rail_x+20, y_min,
                outline=accent, fill="#052235", stipple="gray25", tags="hud"
            )

            # Moving carriage driven by actual Funscript.
            carriage_y = py(pos)
            canvas.create_rectangle(
                rail_x-36, carriage_y-9, rail_x+36, carriage_y+9,
                outline=accent, fill="#063A51", width=2, tags="hud"
            )
            canvas.create_line(
                chamber_x1+12, carriage_y, chamber_x2-12, carriage_y,
                fill=accent, dash=(4,4), tags="hud"
            )
            canvas.create_text(
                rail_x, carriage_y-18, text=f"{pos:0.1f}%",
                fill=accent, font=("DejaVu Sans Mono", 9, "bold"), tags="hud"
            )

            canvas.create_text(chamber_x1+10, rail_top, anchor="w", text="100",
                               fill=muted, font=("DejaVu Sans Mono", 7), tags="hud")
            canvas.create_text(chamber_x1+10, rail_bottom, anchor="w", text="0",
                               fill=muted, font=("DejaVu Sans Mono", 7), tags="hud")

            # Right telemetry stack: responsive 2x2 layout prevents overlap
            # on shorter windows while keeping all four live values visible.
            rx1 = chamber_x2 + 28
            rx2 = w - 32
            top_y = 76
            canvas.create_text(
                rx1, top_y, anchor="w", text="TELEMETRY // LIVE PARAMETERS",
                fill=accent, font=("DejaVu Sans Mono", 9, "bold"), tags="hud"
            )

            gap = 18
            total_w = max(180, rx2 - rx1)
            cell_w = (total_w - gap) / 2.0
            row1_y = top_y + 28
            row2_y = row1_y + 58

            def compact_meter(x1, y, label, value, maximum, display, fill_color=accent):
                x2 = x1 + cell_w
                canvas.create_text(
                    x1, y, anchor="w", text=label,
                    fill=muted, font=("DejaVu Sans Mono", 7), tags="hud"
                )
                canvas.create_text(
                    x2, y, anchor="e", text=display,
                    fill=fill_color, font=("DejaVu Sans Mono", 7, "bold"), tags="hud"
                )
                bar_y = y + 16
                canvas.create_rectangle(
                    x1, bar_y, x2, bar_y + 9,
                    outline=border, fill="#06111D", tags="hud"
                )
                frac = max(0.0, min(1.0, value / maximum if maximum else 0.0))
                canvas.create_rectangle(
                    x1 + 1, bar_y + 1,
                    x1 + 1 + (cell_w - 2) * frac, bar_y + 8,
                    outline="", fill=fill_color, tags="hud"
                )

            col1 = rx1
            col2 = rx1 + cell_w + gap
            compact_meter(col1, row1_y, "CURRENT POSITION", pos, 100.0, f"{pos:0.1f} %")
            compact_meter(col2, row1_y, "BLE RESPONSE", speed, 2.0, f"{speed:0.2f} ×")
            compact_meter(
                col1, row2_y, "AMPLIFICATION", amp, 100.0, f"{amp:0.0f} %",
                warning if amp >= 70 else accent
            )
            compact_meter(
                col2, row2_y, "ACTIVE RANGE",
                max(0.0, max_pos - min_pos), 100.0,
                f"{min_pos:0.0f} → {max_pos:0.0f} %"
            )

            # Mini motion history is shown only when enough vertical room is
            # available. On compact layouts it is omitted instead of colliding
            # with ACTIVE RANGE or the bottom status rail.
            chart_title_y = row2_y + 42
            chart_top = chart_title_y + 16
            chart_bottom = h - 58
            if chart_bottom - chart_top >= 42 and rx2 - rx1 >= 180:
                canvas.create_text(
                    rx1, chart_title_y, anchor="w",
                    text="MOTION WINDOW // ±2.5 s",
                    fill=muted, font=("DejaVu Sans Mono", 7, "bold"), tags="hud"
                )
                canvas.create_rectangle(
                    rx1, chart_top, rx2, chart_bottom,
                    outline=border, fill="#030B14", tags="hud"
                )
                if self.graph_actions:
                    t0 = self.graph_position_ms - 2500
                    t1 = self.graph_position_ms + 2500
                    pts = []
                    for at, p in self.graph_actions:
                        if t0 <= at <= t1:
                            x = rx1 + ((at - t0) / 5000.0) * (rx2 - rx1)
                            y = chart_bottom - (p / 100.0) * (chart_bottom - chart_top)
                            pts.extend((x, y))
                    if len(pts) >= 4:
                        canvas.create_line(*pts, fill=accent, width=2, tags="hud")
                center_x = (rx1 + rx2) / 2
                canvas.create_line(
                    center_x, chart_top, center_x, chart_bottom,
                    fill=success if playing else muted, width=1, tags="hud"
                )

            # Bottom status rail.
            canvas.create_line(20, h-34, w-20, h-34, fill=border, tags="hud")
            canvas.create_text(
                28, h-20, anchor="w",
                text="DIRECT BLE // MPV IPC // LOCAL FUNSCRIPT",
                fill=muted, font=("DejaVu Sans Mono", 7), tags="hud"
            )
            canvas.create_text(
                w-28, h-20, anchor="e",
                text=self._format_clock(self.graph_position_ms/1000.0),
                fill=accent, font=("DejaVu Sans Mono", 8, "bold"), tags="hud"
            )

        self._redraw_hud = redraw
        canvas.bind("<Configure>", redraw)

        # Update instantly when a slider is moved, even while video is stopped.
        for var in (self.min_position, self.max_position, self.speed_multiplier, self.amplification):
            try:
                var.trace_add("write", lambda *_args: self.after_idle(redraw))
            except tk.TclError:
                pass
    ...
    def build_ui(self) -> None:
        ...
        # HUD header
        header = tk.Frame(shell, bg="#03060D", height=78,
                          highlightbackground=COLORS["border"], highlightthickness=1)
        header.pack(side="top", fill="x", padx=12, pady=(12, 6))
        header.pack_propagate(False)
        left = tk.Frame(header, bg="#03060D")
        left.pack(side="left", fill="both", expand=True, padx=16, pady=10)
        tk.Label(left, text=self.tr("title"), bg="#03060D", fg=COLORS["text"],
                 font=("DejaVu Sans Mono", 20, "bold"), anchor="w").pack(anchor="w")
        tk.Label(left, text=self.tr("subtitle"), bg="#03060D", fg=COLORS["muted"],
                 font=("DejaVu Sans Mono", 8), anchor="w").pack(anchor="w", pady=(3, 0))

        right = tk.Frame(header, bg="#03060D")
        right.pack(side="right", padx=12, pady=8)
        self.ble_badge = tk.Label(right, textvariable=self.ble_state, bg=COLORS["panel_alt"], fg=COLORS["muted"],
                                  font=("DejaVu Sans Mono", 9, "bold"), padx=10, pady=6)
        self.ble_badge.pack(side="left", padx=(0, 8))
        tk.Label(right, text=APP_VERSION, bg="#03060D", fg=COLORS["muted"],
                 font=("DejaVu Sans Mono", 8, "bold")).pack(side="right", padx=(8, 0))
        lang = tk.Frame(right, bg=COLORS["panel"], highlightbackground=COLORS["accent"], highlightthickness=1)
        lang.pack(side="right")
        tk.Label(lang, text="LANG", bg=COLORS["panel"], fg=COLORS["accent"],
                 font=("DejaVu Sans Mono", 8, "bold"), padx=8).pack(side="left")
        self.fr_button = tk.Button(lang, text="FR", width=4, command=lambda: self.set_language("fr"),
                                   relief="flat", bd=0, cursor="hand2", font=("DejaVu Sans Mono", 9, "bold"), pady=4)
        self.fr_button.pack(side="left", padx=2, pady=3)
        self.en_button = tk.Button(lang, text="EN", width=4, command=lambda: self.set_language("en"),
                                   relief="flat", bd=0, cursor="hand2", font=("DejaVu Sans Mono", 9, "bold"), pady=4)
        self.en_button.pack(side="left", padx=(0, 3), pady=3)
        self._refresh_language_buttons()

        # Main area
        main = tk.Frame(shell, bg=COLORS["bg"])
        main.pack(side="top", fill="both", expand=True, padx=12, pady=(0, 6))
        ...
        # Media
        media_outer, media = self._make_collapsible(left_col, "media", self.tr("media"))
        media_outer.pack(fill="x", pady=(0, 8))
        media.grid_columnconfigure(1, weight=1)
        tk.Label(media, text=self.tr("video"), bg=COLORS["panel"], fg=COLORS["muted"],
                 font=("Noto Sans", 9)).grid(row=0, column=0, sticky="w", padx=(14, 8), pady=10)
        entry = tk.Entry(media, textvariable=self.video_path, state="readonly", readonlybackground=COLORS["panel_alt"],
                         fg=COLORS["text"], relief="flat", bd=0, font=("DejaVu Sans Mono", 9))
        entry.grid(row=0, column=1, sticky="ew", pady=10, ipady=7)
        tk.Button(media, text="▣  " + self.tr("video_btn"), command=self.choose_video, bg=COLORS["panel_alt"], fg=COLORS["text"],
                  activebackground=COLORS["accent"], activeforeground="#001018", relief="flat", bd=0, padx=10, pady=7,
                  cursor="hand2").grid(row=0, column=2, padx=(8, 4), pady=10)
        tk.Button(media, text="▤  " + self.tr("folder"), command=self.choose_folder, bg=COLORS["panel_alt"], fg=COLORS["text"],
                  activebackground=COLORS["accent"], activeforeground="#001018", relief="flat", bd=0, padx=10, pady=7,
                  cursor="hand2").grid(row=0, column=3, padx=(4, 14), pady=10)
        tk.Label(media, text=self.tr("script"), bg=COLORS["panel"], fg=COLORS["muted"], font=("Noto Sans", 9)).grid(row=1, column=0, sticky="nw", padx=(14, 8), pady=(0, 12))
        self.script_label = tk.Label(media, textvariable=self.script_path, bg=COLORS["panel"], fg=COLORS["muted"],
                                     font=("DejaVu Sans Mono", 8), anchor="w", justify="left", wraplength=720)
        self.script_label.grid(row=1, column=1, columnspan=3, sticky="ew", pady=(0, 12), padx=(0, 14))

        # Motion matrix
        motion_outer, motion = self._make_collapsible(left_col, "motion", self.tr("motion"))
        motion_outer.pack(fill="x", pady=(0, 8))
        motion.grid_columnconfigure(1, weight=1)
        self.add_scale(motion, 0, self.tr("minpos"), self.min_position, 0.0, 0.90, lambda v: f"{float(v)*100:.0f} %")
        self.add_scale(motion, 1, self.tr("maxpos"), self.max_position, 0.10, 1.0, lambda v: f"{float(v)*100:.0f} %")
        self.add_scale(motion, 2, self.tr("ble"), self.speed_multiplier, 0.25, 2.0, lambda v: f"{float(v):.2f} ×")
        self.add_scale(motion, 3, self.tr("amp"), self.amplification, 0, 100,
                       lambda v: f"{float(v):.0f} %   x{1.0 + 2.0*float(v)/100.0:.2f}")

        # Decorative HUD fills the large unused area below Motion Matrix.
        self.hud_design = self._build_hud_design(left_col)
        self.hud_design.pack(fill="both", expand=True, pady=(0, 8))

        # Options
        options_outer, options = self._make_collapsible(right_col, "options", self.tr("options_title"))
        options_outer.pack(fill="x", pady=(0, 8))
        for i, (txt, var) in enumerate(((self.tr("fullscreen"), self.fullscreen),
                                        (self.tr("delete"), self.delete_after_natural_end_var),
                                        (self.tr("autonext"), self.play_next_var),
                                        (self.tr("resume"), self.resume_video_var))):
            cb = tk.Checkbutton(options, text=txt, variable=var, bg=COLORS["panel"], fg=COLORS["text"],
                                selectcolor=COLORS["panel_alt"], activebackground=COLORS["panel"], activeforeground=COLORS["accent_hover"],
                                anchor="w", justify="left", padx=12, pady=5)
            cb.pack(fill="x", padx=4, pady=(4 if i == 0 else 0, 0))
        theme_line = tk.Frame(options, bg=COLORS["panel"])
        theme_line.pack(fill="x", padx=12, pady=10)
        tk.Label(theme_line, text=self.tr("theme") + " //", bg=COLORS["panel"], fg=COLORS["muted"],
                 font=("DejaVu Sans Mono", 8, "bold")).pack(side="left")
        for name in ("Cyber Blue", "Matrix Green", "Purple Neon"):
            b = tk.Button(theme_line, text=name, relief="flat", bd=0, cursor="hand2",
                          bg=COLORS["accent"] if self.theme_name.get() == name else COLORS["panel_alt"],
                          fg="#001018" if self.theme_name.get() == name else COLORS["text"], padx=7, pady=4,
                          command=lambda n=name: self._set_theme(n))
            b.pack(side="left", padx=(8, 0))

        # Playlist
        play_outer, playbody = self._make_collapsible(
            right_col, "playlist", self.tr("playlist_title"), expand_body=True
        )
        play_outer.pack(fill="both", expand=True, pady=(0, 8))
        info = tk.Frame(playbody, bg=COLORS["panel"])
        info.pack(fill="x", padx=10, pady=(8, 4))
        tk.Button(
            info, text="▶  LIRE / PLAY", command=self._play_selected_playlist_video,
            bg=COLORS["accent"], fg="#001018", activebackground=COLORS["accent_hover"],
            activeforeground="#001018", relief="flat", bd=0, cursor="hand2",
            font=("DejaVu Sans Mono", 8, "bold"), padx=10, pady=4
        ).pack(side="left")
        tk.Label(info, textvariable=self.playlist_info, bg=COLORS["panel"], fg=COLORS["accent"],
                 font=("DejaVu Sans Mono", 8, "bold")).pack(side="right")

        self.playlist_selected_label = tk.Label(
            playbody, text="", bg=COLORS["panel"], fg=COLORS["accent_hover"],
            font=("DejaVu Sans Mono", 8, "bold"), anchor="w", padx=10, pady=3
        )
        self.playlist_selected_label.pack(fill="x", padx=2)

        list_frame = tk.Frame(playbody, bg=COLORS["panel"])
        list_frame.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        scroll = tk.Scrollbar(list_frame, orient="vertical")
        scroll.pack(side="right", fill="y")
        self.playlist_listbox = tk.Listbox(list_frame, bg="#040A12", fg=COLORS["text"], selectbackground=COLORS["accent"],
                                           selectforeground="#001018", highlightthickness=1, highlightbackground=COLORS["border"],
                                           relief="flat", bd=0, font=("DejaVu Sans Mono", 9), activestyle="none",
                                           exportselection=False, selectborderwidth=0,
                                           highlightcolor=COLORS["accent"],
                                           yscrollcommand=scroll.set, height=18)
        self.playlist_listbox.pack(side="left", fill="both", expand=True)
        scroll.configure(command=self.playlist_listbox.yview)
        self.playlist_listbox.bind("<<ListboxSelect>>", self._playlist_activate)
        self.playlist_listbox.bind("<Double-Button-1>", self._play_selected_playlist_video)
        self.playlist_listbox.bind("<Return>", self._play_selected_playlist_video)

        # Action strip
        actions = tk.Frame(shell, bg=COLORS["bg"])
        actions.pack(side="top", fill="x", padx=12, pady=(0, 6))
        self.launch_button = tk.Button(actions, text="▶  " + self.tr("engage").replace("▶  ", ""), command=self.launch, state="disabled",
                                       bg=COLORS["accent"], fg="#001018", activebackground=COLORS["accent_hover"], activeforeground="#001018",
                                       relief="flat", bd=0, font=("DejaVu Sans Mono", 11, "bold"), pady=11, cursor="hand2")
        self.launch_button.pack(side="left", fill="x", expand=True, padx=(0, 6))
        self.folder_button = tk.Button(actions, text="▤  " + self.tr("folder_play").replace("▶▶  ", ""), command=self.launch_folder_playlist, state="disabled",
                                       bg=COLORS["panel_alt"], fg=COLORS["text"], relief="flat", bd=0, padx=16, pady=11, cursor="hand2")
        self.folder_button.pack(side="left", padx=6)
        self.next_button = tk.Button(actions, text=self.tr("next"), command=self.next_video, state="disabled",
                                     bg=COLORS["panel_alt"], fg=COLORS["text"], relief="flat", bd=0, padx=16, pady=11, cursor="hand2")
        self.next_button.pack(side="left", padx=6)
        tk.Button(actions, text=self.tr("abort"), command=self.stop, bg="#382028", fg="#ff8797", relief="flat", bd=0,
                  padx=16, pady=11, font=("DejaVu Sans Mono", 9, "bold"), cursor="hand2").pack(side="left", padx=(6, 0))

        # status + timeline
        statusbar = tk.Frame(shell, bg=COLORS["panel_alt"], height=34)
        statusbar.pack(side="bottom", fill="x", padx=12, pady=(0, 6))
        statusbar.pack_propagate(False)
        self.status_label = tk.Label(statusbar, textvariable=self.status, bg=COLORS["panel_alt"], fg=COLORS["muted"],
                                     font=("DejaVu Sans Mono", 8), anchor="w", padx=10)
        self.status_label.pack(side="left", fill="x", expand=True)
        tk.Label(statusbar, textvariable=self.time_text, bg=COLORS["panel_alt"], fg=COLORS["accent"],
                 font=("DejaVu Sans Mono", 9, "bold"), padx=12).pack(side="right")

        graph_frame = tk.Frame(shell, bg="#02050B", height=190, highlightbackground=COLORS["border"], highlightthickness=1)
        graph_frame.pack(side="bottom", fill="x", padx=12, pady=(0, 6))
        ...
        tk.Label(graph_header, text=self.tr("timeline"), bg="#050A13", fg=COLORS["accent"],
                 font=("DejaVu Sans Mono", 8, "bold"), padx=10, anchor="w").pack(side="left", fill="x", expand=True)
        for label, command in ((self.tr("back10"), lambda: self.seek_relative(-10)),
                               (self.tr("pause"), self.toggle_pause),
                               (self.tr("forward10"), lambda: self.seek_relative(10))):
            tk.Button(graph_header, text=label, command=command, bg=COLORS["panel_alt"], fg=COLORS["text"],
                      activebackground=COLORS["accent"], activeforeground="#001018", relief="flat", bd=0,
                      padx=12, pady=3, cursor="hand2").pack(side="left", padx=2, pady=3)
        self.graph_canvas = tk.Canvas(graph_frame, bg="#020711", highlightthickness=0, borderwidth=0, height=145, cursor="hand2")
    ...
    def update_graph_position(self) -> None:
        if self.process is not None and self.process.poll() is None:
            try:
                ...
                self.draw_funscript_graph()
                self._update_time_text()
                if hasattr(self, "_redraw_hud"):
                    self._redraw_hud()
                self.ble_state.set(self.tr("ble_active"))
                if hasattr(self, "ble_badge"):
                    self.ble_badge.configure(fg=COLORS["success"])
                ...
                pass
        if self.process is None or self.process.poll() is not None:
            self.ble_state.set(self.tr("ble_idle"))
            if hasattr(self, "ble_badge"):
                self.ble_badge.configure(fg=COLORS["muted"])
            self._update_time_text()
            if hasattr(self, "_redraw_hud"):
                self._redraw_hud()
    ...
    def choose_folder(self) -> None:
        ...
        self.playlist_selected_video = None
    ...
    def load_config(self) -> None:
        ...
        self.theme_name.set(str(data.get("theme") or self.theme_name.get()))
        theme_map = {"Cyber Blue": ("#00CFFF", "#59E6FF"), "Matrix Green": ("#35F2A1", "#8CFFD0"), "Purple Neon": ("#B56CFF", "#E0B6FF")}
        if self.theme_name.get() in theme_map:
            COLORS["accent"], COLORS["accent_hover"] = theme_map[self.theme_name.get()]
